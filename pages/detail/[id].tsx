import { GithubCounter, ReactionBarSelector } from "@charkour/react-reactions";
...
const Detail = ({ postDetails }: IProps) => {
		...
		const counterObjects = Object.keys(reactionEmojis).flatMap(
			(reactionEmoji) => {
				return response[reactionEmoji]?.map((reaction) => ({
					emoji: reactionEmojis[reactionEmoji],
	...
	const reactionsFromPost = (_post: any) => {
		...
		<div className="flex w-full absolute left-0 top-0 bg-white flex-wrap lg:flex-nowrap">
			...
			<div className="relative w-[1000px] md:w-[900px] lg:w-[700px]">
				<div className="lg:mt-5 mt-2">
					...
					<div className="ml-2">
						<ReactionBarSelector onSelect={handleAddReaction} />
					</div>
					<div>
						<GithubCounter
							counters={mappedReactions}
							onSelect={handleRemoveReaction}
						/>
					</div>
					<hr />
					<Comments
